#ifndef CALCULATOR_PROCESSOR
#define CALCULATOR_PROCESSOR
...
namespace Calculator {
	...
	public:
		...
		CalculatorProcessor(UnaryFuncEval eval, TokenTypes type = TokenTypes::FUNCTION, short prec = 0, bool left = true)
			: CalculatorProcessor(type, prec, left, true) {
			u_eval = eval;
		}

		//constructor for binary functions
		CalculatorProcessor(BinaryFuncEval eval, TokenTypes type = TokenTypes::FUNCTION, short prec = 0, bool left = true)
			: CalculatorProcessor(type, prec, left, false) {
			b_eval = eval;
		}
		...
		UnaryFuncEval u_eval; //callbacks
		BinaryFuncEval b_eval; //callbacks

		TokenTypes type; //types of function		
		short prec; //precedence
		bool left; //is left associative
		bool unary; //is unary function

	private:
		CalculatorProcessor(TokenTypes type, short prec, bool left, bool unary)
			: type(type), prec(prec), left(left), unary(unary), u_eval(nullptr), b_eval(b_eval) {}
	};

	//unary functions
	std::map<std::string, CalculatorProcessor> unary_functions = {
		{ "Sin", CalculatorProcessor(std::sin) },
		{ "Tan", CalculatorProcessor(std::tan) },
		{ "Cos", CalculatorProcessor(std::cos) },
	};

	//binary functions
	std::map<std::string, CalculatorProcessor> binary_functions = {
		{ "+", CalculatorProcessor([](double x, double y) -> double { return x + y; }, TokenTypes::OPERATOR, 2) },
		{ "-", CalculatorProcessor([](double x, double y) -> double { return x - y; }, TokenTypes::OPERATOR, 2) },
		{ "*", CalculatorProcessor([](double x, double y) -> double { return x * y; }, TokenTypes::OPERATOR, 3) },
		{ "/", CalculatorProcessor([](double x, double y) -> double { return x / y; }, TokenTypes::OPERATOR, 3) },
		{ "%", CalculatorProcessor(std::fmod, TokenTypes::OPERATOR, 3) },
		{ "^", CalculatorProcessor(std::pow, TokenTypes::OPERATOR, 4, false) },
	};

	//function names
	std::vector<std::string> functionNames = keys<CalculatorProcessor>(unary_functions, binary_functions);
	...
	class Node {
	public:
		Node(std::string name, bool isCalculatorProcessor)
			: name(name), isCalculatorProcessor(isCalculatorProcessor) {}

		double eval(double x = 0, double y = 0);

		std::string name;
		bool isCalculatorProcessor;
	...
	//function node class
	class CalculatorProcessorNode : public Node {
	public:
		CalculatorProcessorNode(std::string name)
			: Node(name, true) {}

		//set type of function and assign callback
		void setUnary(bool isUnary) {
			this->isUnary = isUnary;

			this->CalculatorProcessor = isUnary ? unary_functions[name] : binary_functions[name];
		}

		//evaluate
		double eval(double x, double y = 0) {
			return this->CalculatorProcessor.eval(x, y);
		}
	...
	Node* parse(RPN rpn) {
		...
		for (std::string item : rpn) {
			if (isNumber(item.c_str())) {
				...
				//function
				CalculatorProcessorNode* f = new CalculatorProcessorNode(item);
	...
	double evaluate(Node* tree) {
		...
		int count = 0;
		for (int i = 0; i < tree->name.size(); i++, count++) {
			if (tree->name[i] == '.') {
				if (count > 1) {
					return 0;
				}
		...
		if (tree->isCalculatorProcessor) {
			CalculatorProcessorNode* ftree = (CalculatorProcessorNode*)tree;
			if (ftree->isUnary) {
				return ftree->eval(evaluate(tree->left));
			}
			else {
				return ftree->eval(evaluate(tree->left), evaluate(tree->right));
			}
		}
		else {
			//number node
	...
	public:
		...
		float SingletonEQ(const char* math) {
			RPN rpn = Calculator::reversePolishNotation(math);
			Node* tree = Calculator::parse(rpn);
			float result = evaluate(tree);
			return result;
		}
	};

	float Calc(const char* math) {
		Singleton& instance = Singleton::Get();
		return instance.SingletonEQ(math);
	}
